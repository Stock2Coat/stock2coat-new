-- CORRECT FUNCTION SIGNATURE BASED ON PostgREST ERROR
-- This matches the exact parameter order PostgREST expects

-- Step 1: Drop any existing function variants (clean slate)
DROP FUNCTION IF EXISTS public.process_inventory_consumption(uuid, text, text, numeric, uuid, text);
...
-- Step 2: Create function with EXACT parameter order from PostgREST error
CREATE OR REPLACE FUNCTION public.process_inventory_consumption(
    p_item_id UUID,
    p_notes TEXT,
    p_project_order TEXT,
    p_quantity NUMERIC,
    p_user_id UUID,
    p_user_name TEXT
)
...
GRANT EXECUTE ON FUNCTION public.process_inventory_consumption(UUID, TEXT, TEXT, NUMERIC, UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.process_inventory_consumption(UUID, TEXT, TEXT, NUMERIC, UUID, TEXT) TO anon;

-- Step 4: Kick the cache
NOTIFY pgrst, 'reload schema';

-- Step 5: Verify deployment
SELECT 
    'SUCCESS: Function deployed with correct signature' as status,
